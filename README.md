...
It can sign transactions locally, and submit the raw transaction data to a remote cico node. The blockchain data is provided by the Insight API (which powers https://explorer.coinicles.tech/), rather than the raw cicod RPC calls.

This library makes it possible to run DApp without the users having to run a full cicod node.

> This library is extracted from the official [CICO web wallet](https://github.com/coiniclesdev/cico-web-wallet).
...
You can decode the raw transaction using `cico-cli`:
